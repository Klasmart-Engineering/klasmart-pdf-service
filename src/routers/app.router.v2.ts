...
appRouter.get(`/:pathPrefix/:pdfName/view.html`, Authorized(AuthType.Any),
    async (request: Request, response: Response, next: NextFunction) => {
    try {
 ...
 */
appRouter.get(`/:pathPrefix/:pdfName/prerender`, Authorized(AuthType.Any), 
    async (request: Request, response: Response, next: NextFunction) => {
    const { pathPrefix, pdfName } = request.params;
    ...
    pdfService.prerenderDocument(pathPrefix, pdfName, accepted, reject);
});

appRouter.get(`/:pathPrefix/:pdfName/validate`, async (request: Request, response: Response, next: NextFunction) => {
    const { pathPrefix, pdfName } = request.params;
    try {
        log.verbose(`Received request to validate PDF ${pdfName}`);
        const start = new Date().valueOf();
        const validationStatus = await pdfService.validateCMSPDF(pathPrefix, pdfName);
...
appRouter.get(`/:pathPrefix/:pdfName/page/:page`, async (request: Request, response: Response, next: NextFunction) => {
    const { pathPrefix, pdfName, page } = request.params;
    ...
    try {
        const stream: Readable = await pdfService.getPDFPage(pdfName, +page, pdfURL, pathPrefix)
...
*/
appRouter.get(`/:pathPrefix/:pdfName/render-page/:page`, async (request: Request, response: Response, next: NextFunction) => {
    ...
    response.contentType('image/jpeg')
    try {
        const stream: Readable = await pdfService.getDirectPageRender(+page, pathPrefix, pdfName)
        stream.pipe(response)
    } catch (err) {
        next(err);
        return;
